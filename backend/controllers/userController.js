...
async function registerUser(req, res) {
    try {
        ...
        if(!name || !email || !password){
            return res.json({succeess:false, message:"Missing Details"});
        }
        if(!validator.isEmail(email)){
            return res.json({succeess:false, message:"Invalid Email"});
        }
        if(password.length < 8){
            return res.json({succeess:false, message:"Enter a strong password"});
        }
