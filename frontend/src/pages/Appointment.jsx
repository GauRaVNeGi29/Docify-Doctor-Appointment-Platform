  ...
  return docInfo && (
    <div>
      <div className='flex flex-col sm:flex-row gap-4'>
        <div>
          <img className='bg-[var(--color-primary)] w-full sm:max-w-72 rounded-lg' src={docInfo.image} alt="" />
        </div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-70'>
        <p>Booking Slots</p>
        ...
        <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4 hide-scrollbar'>
          {docSlots.length && docSlots[slotIndex].map((item, index)=>(
            <p onClick={()=> setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-[var(--color-primary)] text-white' : 'text-gray-500 border border-gray-300'}`} key={index}>
